...
for name in *; do
   VERSION=`cat $name/addon.xml|grep \<addon|grep $name |sed 's/.*version="\([^"]*\)"*.*/\1/g'`
     if [ ! -d "$REPO/addons/matrix/$name/" ]; then
	mkdir $REPO/addons/matrix/$name
     fi
     if [ ! -f "$REPO/addons/matrix/$name/$name-$VERSION.zip" ]; then
       zip -r --exclude=*.git* $REPO/addons/matrix/$name/$name-$VERSION.zip $name -x \*.zip
     fi
